...
def wordScoreLoop(S, scorelist):
    ...
    score = 0
    for let in S:
        score += letterScoreLoop(let, scorelist)
    return score
...
def wordsWithScoreLambda(dct, scores):
    ...
    return map(lambda word: [word, reduce(lambda x, y: x + y, map(lambda letter: letterScore(letter, scrabbleScores), word))], dct )
